...
#
# Proves the frozen public contract and the runtime add/remove story. It
# brings the api up with the dev override and --no-deps (skipping models-init
# so the script can drive installs itself), installs every dialect but the
# last (sorted) via the fetch service, restarts, and runs the per-dialect
# contract checks; asserts the missing dialect 400s, fetch-adds it, restarts,
# asserts 200; fetch-removes it, restarts, asserts 400; adds it back so the
# stack is left complete; and closes with a models-init verify pass over the
# now-complete volume. The image is never rebuilt mid-run; only the volume
# changes.
#
...
if [ "${NOUP:-0}" != "1" ]; then
  ...
  "${COMPOSE[@]}" up -d --build --no-deps api || { echo "compose up failed"; exit 1; }
...
if [ "${NOUP:-0}" != "1" ]; then
  echo "== models-init verify pass (complete volume, no downloads expected) =="
  initlog="$("${COMPOSE[@]}" run --rm models-init 2>&1)"
  case "$initlog" in
    *"verified"*) echo "  ok   models-init verifies the volume" ;;
    *) echo "  FAIL models-init verify pass"; echo "$initlog" | tail -4; FAILED=1 ;;
  esac
fi
