...
# hf_repo, languages, preprocessing, and labels (sub/main per language +
# sub_to_main). The dialect code is the filename stem. Adding a dialect is a
# one-file change: the loader globs the directory, the Dockerfile reads the
# same files to decide which models to download, and nginx renders its routing
# from them at startup. app/languages.json maps language codes to display names.

_DIALECTS_DIR = Path(__file__).parent / "dialects"
_LANGUAGES_PATH = Path(__file__).parent / "languages.json"
...
# code -> human-readable language name, the single source for label languages
LANGUAGE_NAMES: dict[str, str] = dict(_load_json(_LANGUAGES_PATH))
...
SUPPORTED_LANGUAGES: frozenset[str] = frozenset(_DIALECTS_CONFIG[DIALECT]["languages"])
...
async def get_classification(text: str, lang: str = "ar") -> ClassificationResult:
