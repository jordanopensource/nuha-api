...
One stateless image. The ``{dialect}`` path segment selects a model loaded
from the models volume (see app.registry, which decides the servable set at
startup); inference, preprocessing, caching, and labels run in-process (see
app.classifier). Operators change the served dialects with the fetch command
plus a restart.
"""
