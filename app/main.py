...
_LANG_PARAM_DESC = (
    ...
    + ", ".join(
        f"'{code}' ({name})" for code, name in LANGUAGE_NAMES.items() if code in SUPPORTED_LANGUAGES
    )
    + "."
)
...
def _validate_lang(lang: str) -> None:
    """Raise 422 if the requested language is not supported by this dialect."""
...
async def classify_single(
    ...
    lang: Annotated[str, Query(description=_LANG_PARAM_DESC)] = "ar",
