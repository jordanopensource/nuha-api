...
Models are data mounted at runtime rather than image layers. One image serves
every dialect and carries no model; the models live on a Docker volume, one
directory per dialect, populated by models-init at startup and managed with
the fetch command. The api scans that volume once at startup, so the operator
flow for changing what the stack serves is:
...
# first (the initContainer pattern): it verifies the volume and downloads any
# missing model, and only then does the api start. A cold first boot therefore
# needs nothing beyond this one command.
...
revision differs from the repo head. `docker compose restart api` does not
re-run the init service; the manual flow below stays the way to change the
...
By default a cold `up` runs models-init, which installs every shipped dialect
before the api starts. On a box sized for a single model, skip the init
service with `--no-deps` and install only what you want:
...
A later plain `docker compose up -d` would run models-init and fill the
volume back up; either keep using `--no-deps`, or narrow the init service's
command in `compose.yml` to the dialects this deployment serves.
...
`is_valid` is `false` when the text does not survive preprocessing (for
example it is empty after cleaning, or it is only emojis); the three other
...
The volume holds one directory per dialect (`/models/<code>/`), containing
the installed `dialect.json`, the model snapshot, and an `.install.json`
revision record. The api mounts it read-only and scans it once at startup;
the fetch tooling (models-init at startup, the manual fetch service on
demand) is the only writer and the only piece needing network egress to
HuggingFace. The startup scan ignores
...
Every install records the resolved model revision in the directory's
`.install.json`, which is what lets `ensure --update` (and the models-init
service) distinguish a current install from a stale one without re-downloading
blindly. `ensure` never prunes: a dialect you installed manually stays until
you `remove` it.

Model revisions are unpinned by default, so a re-fetch follows the HF repo's
head without a code change (the trade-off the old build-time bake made, now
without the rebuild); every install still records the exact revision it got,
and `--revision` pins one. `restart` drains: the 160s
...
Pinning the per-inference threads avoids a trap the old torch build hit.
Left to its default, ONNX Runtime sizes its intra-op thread pool to the host
core count and ignores the Docker CPU cap; under that cap the kernel
throttles the container and every inference slows down. Pinning
`ORT_INTRA_OP_THREADS` to 1 keeps each inference on a single core, so the
slots run one per core with no oversubscription.
...
is a PVC, the `models-init` service is the initContainer verbatim, and
`/ready` is the readiness probe.

## Adding a dialect

Adding a dialect means installing one directory on the volume and restarting;
application code, images, compose, and CI stay untouched.
...
and verifies each wheel's hash, making the build reproducible and
tamper-evident.
...
scripts/
  fetch_models.py      Install/ensure/remove/list models on the volume (the
                       models-init and fetch services' entrypoint)
...
- Status codes: 200 for a served request and 422 for invalid input; 400
  ...
  cases. 422 bodies keep their `detail` list shape but do not echo the
...
- Explore container orchestration and autoscaling. The service is already the
  ...
  with `models-init` carried over as the initContainer against a PVC.
