...
def test_alias_colliding_with_a_canonical_code_flagged():
    """An alias equal to any canonical code would reroute that lang to the
    wrong language's labels at request time; the schema refuses it."""
    cfg = _valid_cfg()
    langs = list(cfg["languages"])
    if len(langs) < 2:
        pytest.skip("needs two languages to collide")
    first, second = langs[0], langs[1]
    cfg["languages"][first].setdefault("aliases", []).append(second)
    problems = validate("x", cfg)
    assert any("collides with a canonical" in p for p in problems), problems


def test_duplicate_alias_across_languages_flagged():
    cfg = _valid_cfg()
    langs = list(cfg["languages"])
    if len(langs) < 2:
        pytest.skip("needs two languages to duplicate across")
    first, second = langs[0], langs[1]
    cfg["languages"][first].setdefault("aliases", []).append("zz")
    cfg["languages"][second].setdefault("aliases", []).append("zz")
    problems = validate("x", cfg)
    assert any("declared by both" in p for p in problems), problems
