...
class TestActiveConfig:
    ...
    def test_active_config_has_label_dicts(self):
        ...
        assert isinstance(ACTIVE_CONFIG.sub_labels["ar"], dict)
        assert isinstance(ACTIVE_CONFIG.main_labels["ar"], dict)
...
class TestLabelSelection:
    ...
    def test_ar_selects_arabic_labels(self):
        ...
        assert len(ACTIVE_CONFIG.sub_labels["ar"]) > 0
        assert len(ACTIVE_CONFIG.main_labels["ar"]) > 0

    def test_en_selects_english_labels(self):
        """lang='en' selects English label dicts (non-empty)."""
        from app.classifier import ACTIVE_CONFIG

        assert len(ACTIVE_CONFIG.sub_labels["en"]) > 0
        assert len(ACTIVE_CONFIG.main_labels["en"]) > 0
    ...
    def test_ar_en_labels_have_same_keys(self):
        ...
        assert set(ACTIVE_CONFIG.sub_labels["ar"].keys()) == set(
            ACTIVE_CONFIG.sub_labels["en"].keys()
        )
        assert set(ACTIVE_CONFIG.main_labels["ar"].keys()) == set(
            ACTIVE_CONFIG.main_labels["en"].keys()
        )
...
class TestDialectAndLanguageConfig:
    ...
    def test_supported_languages_includes_ar_en(self):
        """SUPPORTED_LANGUAGES always includes ar and en."""
        from app.classifier import SUPPORTED_LANGUAGES
