...
class TestExecutorManagement:
    ...
    def test_reset_gate_replaces_the_module_gate(self):
        """The lifespan resets the gate at startup so a fresh boot never
        inherits a semaphore bound to a dead loop or a stale in-flight
        count."""
        import app.classifier as clf

        before = clf._inference_gate
        clf.reset_gate()
        try:
            assert clf._inference_gate is not before
            assert clf._inference_gate.try_admit() is True
        finally:
            clf.reset_gate()
