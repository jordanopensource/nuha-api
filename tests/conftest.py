...
LANGUAGES_PATH = APP_DIR / "languages.json"
...
def dialects_data() -> dict:
    """Dialect config in the legacy document shape: {languages, dialects}.

    Reconstructed from the per-dialect files (minus their labels) plus
    languages.json, so structure tests keep working against one fixture.
    """
    dialects = {
...
def _make_mock_predict_single(dialect: str, labels_data: dict):
    ...
    ar_sub = {int(k): v for k, v in entry["sub"]["ar"].items()}
    en_sub = {int(k): v for k, v in entry["sub"]["en"].items()}
    ar_main = {int(k): v for k, v in entry["main"]["ar"].items()}
    en_main = {int(k): v for k, v in entry["main"]["en"].items()}
    ckb_sub = {int(k): v for k, v in entry["sub"].get("ckb", {}).items()}
    ckb_main = {int(k): v for k, v in entry["main"].get("ckb", {}).items()}
    ...
    def mock_predict(text, loaded, cfg, lang):
        ...
        conf = 0.95
        if lang == "ckb":
            sub_l, main_l = ckb_sub, ckb_main
        elif lang == "ar":
            sub_l, main_l = ar_sub, ar_main
        else:
            sub_l, main_l = en_sub, en_main
        return ClassificationResult(
            is_valid=True,
...
def test_client(labels_data):
    ...
    async def mock_get_classification(text, lang="ar"):
        loaded = _make_mock_loaded_model()
        return mock_single(text, loaded, clf.ACTIVE_CONFIG, lang)

    async def mock_get_classifications_batch(texts, lang="ar"):
        loaded = _make_mock_loaded_model()
